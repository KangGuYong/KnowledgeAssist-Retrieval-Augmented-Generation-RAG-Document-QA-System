...
def _ocr_image_block(
    page, block: dict, ocr: SupportsImageOcr, cache: dict[str, str]
) -> str:
    """Run OCR on one image block, reusing results for repeated images."""
    import fitz

    raw = block.get("image")
    key = hashlib.md5(raw).hexdigest() if raw else None
...
def _extract_page(
    ...
    ``cache`` maps image content to already recognised text, so a logo or a
    header image repeated across pages is only read once.
    """
    stats = _OcrStats()
    ...
    if looks_scanned or too_many_images:
        try:
            text = (ocr.image_to_text(_render(page)) or "").strip()
    ...
    for block in image_blocks:
        stats.images += 1
        try:
            text = _ocr_image_block(page, block, ocr, cache)
        except Exception as e:
            logger.warning("OCR failed on page %s: %s", page_number, e)
            text = ""
...
def extract_pages(
    ...
    with fitz.open(file_path) as document:
        for index, page in enumerate(document):
            pages.append(_extract_page(page, index + 1, ocr, cache))

...
현재 배치에서는 **중위권 청크가 전부 이 취약 구간에 들어간다.** 실제 설정은
`chunk_size=1000`, `retrieval_k=5`(`backend/.env`가 클래스 기본값 10을 덮어씀)이므로
컨텍스트는 약 5천 자다.
...
**이 프로젝트의 실제 값은 `retrieval_k = 5`다.** `config.py`의 클래스 기본값은
10이지만 `backend/.env`가 5로 덮어쓰며, 런타임에 로딩되는 값이 그쪽이다. 5는
홀수이므로 **1등 청크가 컨텍스트 맨 앞에 온다** — Liu et al.의 U자 곡선에서
시작 위치가 끝 위치보다 유리하므로, 실제 환경은 유리한 쪽에 놓인다.

다만 이 유리함은 **설정값에 우연히 의존한다.** 누군가 `RETRIEVAL_K`를 짝수로
바꾸면 방향이 조용히 뒤집혀 1등이 맨 뒤로 간다. 코드가 이를 막지도, 경고하지도
않는다. 알고리즘을 직접 소유하지 않기로 한 결정의 대가이며, 6절에 한계로 남긴다.
...
- **배치 방향이 `RETRIEVAL_K`의 홀짝에 조용히 묶여 있다.** 현재 실제 값 5(홀수)에서는
  1등 청크가 맨 앞에 오는 유리한 방향이지만, 짝수로 바꾸면 뒤집힌다. 경고도 검증도
  없다. `LongContextReorder`를 그대로 쓰기로 한 결정에서 나온 결합이며, 7.2절의
  계약 테스트가 이 동작 자체는 고정한다.
- **`retrieval_k=5`, `chunk_size=1000`이면 컨텍스트는 약 5천 자다.** Liu et al.이
  관찰한 U자 곡선은 이보다 훨씬 긴 컨텍스트에서 두드러진다. 이 길이에서 효과가
  얼마나 되는지는 알 수 없으며, 7.3절의 A/B가 판단할 몫이다.

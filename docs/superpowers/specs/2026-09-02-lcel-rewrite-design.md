...
| 항목 | 근거 |
|---|---|
| `[출처: 문서명]` 규칙 정리 | §1.2 발견 1. 프롬프트에서 규칙을 빼거나, `document_prompt`에 `filename`/`page`를 실어 규칙을 지킬 수 있게 만들거나 — 택일. 후자는 컨텍스트 토큰을 늘린다. |
| 조건부 OCR 주의문 | 검색된 청크에 `settings.ocr_block_prefix`가 없을 때도 OCR 경고가 항상 들어간다. 토큰 낭비이자 불필요한 헤지 유도. 컨텍스트를 보고 붙이면 된다. |
| LLM 호출 재시도 | 원격 Ollama(`ollama_base_url`)의 일시적 실패가 그대로 500이 된다. 체인을 소유하면 감쌀 지점이 명확하다. |
| `max_tokens` 연결 또는 삭제 | §1.2 발견 2. 쓰거나 지우거나 — 둘 다 아닌 현재 상태만은 안 된다. |
| 한국어 condense 프롬프트 | §3.1. 효과는 측정 대상이지 자명하지 않다. |
